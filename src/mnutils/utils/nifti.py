...
def orient_nifti(
    ...
    if caching:
        logger.debug(
            "Using caching for NIfTI data. This may use more memory but can speed up processing for repeat access."
        )
        caching_param = "fill"
    else:
        logger.debug(
            "Not using caching for NIfTI data. This may use less memory but can slow down processing for repeat access."
        )
...
def apply_half_voxel_shift(
    ...
    raise DeprecationWarning(
        "This function should not be used as half voxel shifts are usually bad and a result of bad affine definition"
...
def downsample_to_coverage(
    ...
    **kwargs,
    ...
    if src_affine is None or tgt_affine is None or tgt_shape is None:
        raise ValueError(
            "Source and target affines and target shape must be provided. Either pass target data as Nifti/GESeries or provide affines and shape directly."
        )

    # Generally we do not need to do a half voxel shift as our offsets center the samples within the voxels
    ...
    if super_sampling:
        ...
        logger.warning(
            "No supersampling. This may lead to inaccurate coverage estimates, especially for small voxels or large transformations."
        )
